...
public class CfsSubsetEval extends ASEvaluation implements SubsetEvaluator, ThreadSafe, OptionHandler, TechnicalInformationHandler {
  ...
  public String preComputeCorrelationMatrixTipText() {
    return "Precompute the full correlation matrix at the outset, " + "rather than compute correlations lazily (as needed) " + "during the search. Use this in conjuction with "
  ...
  public String locallyPredictiveTipText() {
    return "Identify locally predictive attributes. Iteratively adds " + "attributes with the highest correlation with the class as long "
        + "as there is not already an attribute in the subset that has a " + "higher correlation with the attribute in question";
  }
              ...
              public Void call() throws Exception {
                ...
                if (CfsSubsetEval.this.m_debug) {
                  System.err.println("Percentage of correlation matrix computed: "
                      + Utils.doubleToString(((double) CfsSubsetEval.this.m_numFilled.get() / CfsSubsetEval.this.m_numEntries * 100.0), 2) + "%");
                }
  ...
  public int[] postProcess(final int[] attributeSet) throws Exception {
    ...
    if (this.m_debug) {
      System.err.println(
          "Percentage of correlation matrix computed " + "over the search: " + Utils.doubleToString(((double) this.m_numFilled.get() / this.m_numEntries * 100.0), 2) + "%");
    }
