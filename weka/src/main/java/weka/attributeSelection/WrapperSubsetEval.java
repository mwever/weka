...
public class WrapperSubsetEval extends ASEvaluation implements SubsetEvaluator, OptionHandler, TechnicalInformationHandler {
	...
	protected static class PluginTag extends Tag {
		 ...
		 */
		public PluginTag(final int ID, final AbstractEvaluationMetric metric, final String statisticName) {
	...
	public Enumeration<Option> listOptions() {
		...
		if ((this.m_BaseClassifier != null) && (this.m_BaseClassifier instanceof OptionHandler)) {
			...
			newVector.addAll(Collections.list(((OptionHandler) this.m_BaseClassifier).listOptions()));
		}
	...
	public String[] getOptions() {
		...
		if ((this.m_BaseClassifier != null) && (this.m_BaseClassifier instanceof OptionHandler)) {
			classifierOptions = ((OptionHandler) this.m_BaseClassifier).getOptions();
		}
	...
	public Capabilities getCapabilities() {
		...
		if (this.m_evaluationMeasure.getID() != EVAL_ACCURACY && this.m_evaluationMeasure.getID() != EVAL_FMEASURE && this.m_evaluationMeasure.getID() != EVAL_AUC && this.m_evaluationMeasure.getID() != EVAL_AUPRC
	...
	public double evaluateSubset(final BitSet subset) throws Exception {
			...
			default:
				...
				if (pluginMetric instanceof InformationRetrievalEvaluationMetric) {
					if (this.m_IRClassVal < 0) {
						...
						repError[i] = ((InformationRetrievalEvaluationMetric) pluginMetric).getStatistic(statName, this.m_IRClassVal);
					}
				} else {
		...
		case EVAL_RMSE:
		case EVAL_MAE:
			if (this.m_trainInstances.classAttribute().isNominal() && (this.m_evaluationMeasure.getID() == EVAL_DEFAULT || this.m_evaluationMeasure.getID() == EVAL_ACCURACY)) {
	 ...
	 */
	private boolean repeat(final double[] repError, final int entries) {
		int i;
		double mean = 0;
		double variance = 0;
		...
		if ((variance / mean) > this.m_threshold) {
			return true;
		}
