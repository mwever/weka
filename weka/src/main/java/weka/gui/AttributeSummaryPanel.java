...
public class AttributeSummaryPanel extends JPanel {
	...
	public AttributeSummaryPanel() {
		...
		this.m_WeightLab.setBorder(BorderFactory.createEmptyBorder(0, 10, 0, 0));
		gbC = new GridBagConstraints();
		gbC.anchor = GridBagConstraints.EAST;
		gbC.fill = GridBagConstraints.HORIZONTAL;
		gbC.gridy = 0;
		gbC.gridx = 2;
		gbL.setConstraints(this.m_WeightLab, gbC);
		simple.add(this.m_WeightLab);
		gbC = new GridBagConstraints();
		gbC.anchor = GridBagConstraints.WEST;
		gbC.fill = GridBagConstraints.HORIZONTAL;
		gbC.gridy = 0;
		...
		gbC.weightx = 100;
		gbC.gridwidth = 3;
		gbL.setConstraints(this.m_AttributeWeightLab, gbC);
		simple.add(this.m_AttributeWeightLab);
		this.m_AttributeWeightLab.setBorder(BorderFactory.createEmptyBorder(0, 5, 0, 10));
		...
		this.add(simple, BorderLayout.NORTH);
		this.add(new JScrollPane(this.m_StatsTable), BorderLayout.CENTER);
		this.m_StatsTable.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	}
	...
	public void setInstances(final Instances inst) {
		...
		this.m_AttributeWeightLab.setText(NO_SOURCE);
		...
		for (int i = 1; i < this.m_Instances.numInstances(); i++) {
			if (this.m_Instances.instance(i).weight() != w) {
	 ...
	 */
	protected void setDerived(final int index) {
	 ...
	 * @param index the index of the attribute
	 */
	protected void setTable(final AttributeStats as, final int index) {
	 ...
	 * @param index the index of the attribute
	 */
	protected void setHeader(final int index) {
		...
		if (att.weight() != 1.0) {
			this.m_AttributeWeightLab.setText(Utils.doubleToString(att.weight(), 3));
			this.m_AttributeWeightLab.setVisible(true);
			this.m_WeightLab.setVisible(true);
		} else {
			this.m_AttributeWeightLab.setVisible(false);
			this.m_WeightLab.setVisible(false);
		}
