...
public class AttributeVisualizationPanel extends PrintablePanel {
	 ...
	 */
	public AttributeVisualizationPanel(final boolean showColouringOption) {
	...
	public int getColoringIndex() {
		return this.m_classIndex; // m_colorAttrib.getSelectedIndex();
	}
	 ...
	 */
	public void calcGraph(final int panelWidth, final int panelHeight) {

		synchronized (this.m_locker) {
			this.m_threadRun = true;
			if (this.m_as.nominalWeights != null) {
				this.m_hc = new BarCalc(panelWidth, panelHeight);
				this.m_hc.setPriority(Thread.MIN_PRIORITY);
				this.m_hc.start();
	...
	private class BarCalc extends Thread {
		...
		public BarCalc(final int panelWidth, final int panelHeight) {
		...
		public void run() {
			synchronized (AttributeVisualizationPanel.this.m_locker) {
				...
				if ((AttributeVisualizationPanel.this.m_classIndex >= 0) && (AttributeVisualizationPanel.this.m_data.attribute(AttributeVisualizationPanel.this.m_classIndex).isNominal())) {
					...
					histClassCounts = new SparseInstance[AttributeVisualizationPanel.this.m_data.attribute(AttributeVisualizationPanel.this.m_attribIndex).numValues()];
					...
					for (int k = 0; k < AttributeVisualizationPanel.this.m_data.numInstances(); k++) {
						...
						if (!AttributeVisualizationPanel.this.m_data.instance(k).isMissing(AttributeVisualizationPanel.this.m_attribIndex)) {
							// check to see if we need to allocate some space here
							if (AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) != tempAttValueIndex) {
							...
							if (AttributeVisualizationPanel.this.m_data.instance(k).isMissing(AttributeVisualizationPanel.this.m_classIndex)) {
								 ...
								 */
								tempClassCounts[0] += AttributeVisualizationPanel.this.m_data.instance(k).weight();
							} else {
								tempClassCounts[(int) AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_classIndex) + 1] += AttributeVisualizationPanel.this.m_data.instance(k).weight();
					...
					double histCounts[];
					histCounts = new double[AttributeVisualizationPanel.this.m_data.attribute(AttributeVisualizationPanel.this.m_attribIndex).numValues()];
					...
					for (int k = 0; k < AttributeVisualizationPanel.this.m_data.numInstances(); k++) {
						if (!AttributeVisualizationPanel.this.m_data.instance(k).isMissing(AttributeVisualizationPanel.this.m_attribIndex)) {
							histCounts[(int) AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex)] += AttributeVisualizationPanel.this.m_data.instance(k).weight();
						}
					}
					AttributeVisualizationPanel.this.m_threadRun = false;
					AttributeVisualizationPanel.this.m_displayCurrentAttribute = true;
		...
		public void run() {
			synchronized (AttributeVisualizationPanel.this.m_locker) {
				if ((AttributeVisualizationPanel.this.m_classIndex >= 0) && (AttributeVisualizationPanel.this.m_data.attribute(AttributeVisualizationPanel.this.m_classIndex).isNominal())) {
					...
					intervalWidth = 3.49 * AttributeVisualizationPanel.this.m_as.numericStats.stdDev * Math.pow(AttributeVisualizationPanel.this.m_data.numInstances(), -1 / 3D);
					...
					intervals = Math.max(1, (int) Math.round((AttributeVisualizationPanel.this.m_as.numericStats.max - AttributeVisualizationPanel.this.m_as.numericStats.min) / intervalWidth));
					...
					double barRange = (AttributeVisualizationPanel.this.m_as.numericStats.max - AttributeVisualizationPanel.this.m_as.numericStats.min) / histClassCounts.length;
					...
					for (int k = 0; k < AttributeVisualizationPanel.this.m_data.numInstances(); k++) {
						...
						try {
							if (!AttributeVisualizationPanel.this.m_data.instance(k).isMissing(AttributeVisualizationPanel.this.m_attribIndex)) {
								...
								if (t == 0) {
									if (AttributeVisualizationPanel.this.m_data.instance(k).isMissing(AttributeVisualizationPanel.this.m_classIndex)) {
										histClassCounts[t][0] += AttributeVisualizationPanel.this.m_data.instance(k).weight();
									} else {
										histClassCounts[t][(int) AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_classIndex) + 1] += AttributeVisualizationPanel.this.m_data.instance(k).weight();
										// if(histCounts[t]>m_maxValue)
										// m_maxValue = histCounts[t];
									}
								} else {
									if (AttributeVisualizationPanel.this.m_data.instance(k).isMissing(AttributeVisualizationPanel.this.m_classIndex)) {
										histClassCounts[t - 1][0] += AttributeVisualizationPanel.this.m_data.instance(k).weight();
									} else {
										histClassCounts[t - 1][(int) AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_classIndex) + 1] += AttributeVisualizationPanel.this.m_data.instance(k)
												.weight();
							...
							System.out.println("t:" + (t) + " barRange:" + barRange + " histLength:" + histClassCounts.length + " value:"
									+ AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) + " min:" + AttributeVisualizationPanel.this.m_as.numericStats.min + " sumResult:"
									+ (AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) - AttributeVisualizationPanel.this.m_as.numericStats.min) + " divideResult:"
									+ (float) ((AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) - AttributeVisualizationPanel.this.m_as.numericStats.min) / barRange)
									+ " finalResult:"
									+ Math.ceil((float) ((AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) - AttributeVisualizationPanel.this.m_as.numericStats.min) / barRange)));
						}
					}
					for (double[] histClassCount : histClassCounts) {
						...
						if (AttributeVisualizationPanel.this.m_maxValue < sum) {
					...
					double barRange = (AttributeVisualizationPanel.this.m_as.numericStats.max - AttributeVisualizationPanel.this.m_as.numericStats.min) / histCounts.length;
					...
					for (int k = 0; k < AttributeVisualizationPanel.this.m_data.numInstances(); k++) {
						...
						try {
							...
							t = (int) Math.ceil((float) ((AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) - AttributeVisualizationPanel.this.m_as.numericStats.min) / barRange));
							if (t == 0) {
								histCounts[t] += AttributeVisualizationPanel.this.m_data.instance(k).weight();
								if (histCounts[t] > AttributeVisualizationPanel.this.m_maxValue) {
									AttributeVisualizationPanel.this.m_maxValue = histCounts[t];
								}
							} else {
								histCounts[t - 1] += AttributeVisualizationPanel.this.m_data.instance(k).weight();
								if (histCounts[t - 1] > AttributeVisualizationPanel.this.m_maxValue) {
									AttributeVisualizationPanel.this.m_maxValue = histCounts[t - 1];
								}
							}
						} catch (ArrayIndexOutOfBoundsException ae) {
							ae.printStackTrace();
							System.out.println("t:" + (t) + " barRange:" + barRange + " histLength:" + histCounts.length + " value:" + AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex)
									+ " min:" + AttributeVisualizationPanel.this.m_as.numericStats.min + " sumResult:"
									+ (AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) - AttributeVisualizationPanel.this.m_as.numericStats.min) + " divideResult:"
									+ (float) ((AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) - AttributeVisualizationPanel.this.m_as.numericStats.min) / barRange)
									+ " finalResult:"
									+ Math.ceil((float) ((AttributeVisualizationPanel.this.m_data.instance(k).value(AttributeVisualizationPanel.this.m_attribIndex) - AttributeVisualizationPanel.this.m_as.numericStats.min) / barRange)));
	...
	public String getToolTipText(final MouseEvent ev) {
		...
		if (this.m_as != null && this.m_as.nominalWeights != null) { // if current attrib is
			// nominal

			float intervalWidth = this.getWidth() / (float) this.m_as.nominalWeights.length;
			double heightRatio;
			int barWidth, x = 0;
			...
			if (this.getWidth() - (this.m_as.nominalWeights.length * barWidth + (int) ((Math.floor(intervalWidth * 0.2F)) < 1 ? 1 : (Math.floor(intervalWidth * 0.2F))) * this.m_as.nominalWeights.length) > 2) {
				...
				// centered.
				x += (this.getWidth() - (this.m_as.nominalWeights.length * barWidth + (int) ((Math.floor(intervalWidth * 0.2F)) < 1 ? 1 : (Math.floor(intervalWidth * 0.2F))) * this.m_as.nominalWeights.length)) / 2;
			}

			for (int i = 0; i < this.m_as.nominalWeights.length; i++) {
				heightRatio = (this.getHeight() - (double) this.m_fm.getHeight()) / this.m_maxValue;

				// if our mouse is on a bar then return the count of this bar in our
				// barplot
				if (ev.getX() >= x && ev.getX() <= x + barWidth && ev.getY() >= this.getHeight() - Math.round(this.m_as.nominalWeights[i] * heightRatio)) {
		...
		} else if (this.m_threadRun == false && // if attrib is numeric
				(this.m_histBarCounts != null || this.m_histBarClassCounts != null)) {
			...
			if ((this.m_classIndex >= 0) && (this.m_data.attribute(this.m_classIndex).isNominal())) {
				...
				barWidth = ((this.getWidth() - 6) / this.m_histBarClassCounts.length) < 1 ? 1 : ((this.getWidth() - 6) / this.m_histBarClassCounts.length);
				...
				if (ev.getX() - x >= 0) {
					...
					if (temp == 0) { // handle the special case temp==0. see footnote 1
						...
						return ("<html><center><font face=Dialog size=-1>" + Utils.doubleToString(sum, 3) + "<br>" + "[" + Utils.doubleToString(bar + this.m_barRange * temp, 3) + ", "
								+ Utils.doubleToString((bar + this.m_barRange * (temp + 1)), 3) + "]" + "</font></center></html>");
					} else if (temp < this.m_histBarClassCounts.length) { // handle case
						// temp!=0
						double sum = 0;
				...
				barWidth = ((this.getWidth() - 6) / this.m_histBarCounts.length) < 1 ? 1 : ((this.getWidth() - 6) / this.m_histBarCounts.length);
				...
				if (ev.getX() - x >= 0) {
					...
					if (temp == 0) {
						return ("<html><center><font face=Dialog size=-1>" + this.m_histBarCounts[0] + "<br>" + "[" + Utils.doubleToString(bar + this.m_barRange * temp, 3) + ", "
								+ Utils.doubleToString((bar + this.m_barRange * (temp + 1)), 3) + "]" + "</font></center></html>");
					} else if (temp < this.m_histBarCounts.length) {
						return ("<html><center><font face=Dialog size=-1>" + this.m_histBarCounts[temp] + "<br>" + "(" + Utils.doubleToString(bar + this.m_barRange * temp, 3) + ", "
								+ Utils.doubleToString((bar + this.m_barRange * (temp + 1)), 3) + "]" + "</font></center></html>");
					}
				}
			}
	...
	public void paintComponent(final Graphics g) {
		...
		if (this.m_as != null) { // If calculations have been done and histogram/barplot
			if (!this.m_doneCurrentAttribute && !this.m_threadRun) {
				this.calcGraph(this.getWidth(), this.getHeight());
			}
			if (this.m_threadRun == false && this.m_displayCurrentAttribute) { // calculation
				// thread is not
				// running
				int buttonHeight = 0;
				...
				if (this.m_as.nominalWeights != null && (this.m_histBarClassCounts != null || this.m_histBarCounts != null)) {
					...
					if ((this.m_classIndex >= 0) && (this.m_data.attribute(this.m_classIndex).isNominal())) {
						...
						intervalWidth = (this.getWidth() / (float) this.m_histBarClassCounts.length);
						...
						// a "centered" barplot.
						if (this.getWidth() - (this.m_histBarClassCounts.length * barWidth + (int) ((Math.floor(intervalWidth * 0.2F)) < 1 ? 1 : (Math.floor(intervalWidth * 0.2F))) * this.m_histBarClassCounts.length) > 2) {
							...
							// component.
							x += (this.getWidth() - (this.m_histBarClassCounts.length * barWidth + (int) ((Math.floor(intervalWidth * 0.2F)) < 1 ? 1 : (Math.floor(intervalWidth * 0.2F))) * this.m_histBarClassCounts.length)) / 2;
						}
						...
						for (SparseInstance m_histBarClassCount : this.m_histBarClassCounts) {
							...
							// class
							// ComboBox.
							heightRatio = (this.getHeight() - (double) this.m_fm.getHeight() - buttonHeight) / this.m_maxValue;
							y = this.getHeight();
							...
							if (this.m_fm.stringWidth(Utils.doubleToString(sum, 1)) < intervalWidth) {
					...
					// else if class attribute is numeric or not set then draw black bars.
						...
						intervalWidth = (this.getWidth() / (float) this.m_histBarCounts.length);
						...
						if (this.getWidth() - (this.m_histBarCounts.length * barWidth + (int) ((Math.floor(intervalWidth * 0.2F)) < 1 ? 1 : (Math.floor(intervalWidth * 0.2F))) * this.m_histBarCounts.length) > 2) {
							x += (this.getWidth() - (this.m_histBarCounts.length * barWidth + (int) ((Math.floor(intervalWidth * 0.2F)) < 1 ? 1 : (Math.floor(intervalWidth * 0.2F))) * this.m_histBarCounts.length)) / 2;
						}
						...
						for (double m_histBarCount : this.m_histBarCounts) {
							...
							heightRatio = (this.getHeight() - (float) this.m_fm.getHeight() - buttonHeight) / this.m_maxValue;
							...
							if (this.m_fm.stringWidth(Utils.doubleToString(m_histBarCount, 1)) < intervalWidth) {
					...
					if ((this.m_classIndex >= 0) && (this.m_data.attribute(this.m_classIndex).isNominal())) {
						...
						for (SparseInstance m_histBarClassCount : this.m_histBarClassCounts) {
							if (m_histBarClassCount != null) {
								...
								// at
								// the bottom
								heightRatio = (this.getHeight() - (float) this.m_fm.getHeight() - buttonHeight - 19) / this.m_maxValue;
								y = this.getHeight() - 19;
						...
						g.drawLine(x, this.getHeight() - 17, (barWidth == 1) ? x + barWidth * this.m_histBarClassCounts.length - 1 : x + barWidth * this.m_histBarClassCounts.length, this.getHeight() - 17); // axis
						// line --
						// see
						// footnote
						// 2.
						g.drawLine(x, this.getHeight() - 16, x, this.getHeight() - 12); // minimum
																						// line
						g.drawString(Utils.doubleToString(this.m_as.numericStats.min, 2), x, this.getHeight() - 12 + this.m_fm.getHeight()); // minimum value
						g.drawLine(x + (barWidth * this.m_histBarClassCounts.length) / 2, this.getHeight() - 16, x + (barWidth * this.m_histBarClassCounts.length) / 2, this.getHeight() - 12); // median line
						...
						g.drawString(Utils.doubleToString(this.m_as.numericStats.max / 2 + this.m_as.numericStats.min / 2, 2),
								x + (barWidth * this.m_histBarClassCounts.length) / 2 - this.m_fm.stringWidth(Utils.doubleToString(this.m_as.numericStats.max / 2 + this.m_as.numericStats.min / 2, 2)) / 2,
								this.getHeight() - 12 + this.m_fm.getHeight()); // median value
						g.drawLine((barWidth == 1) ? x + barWidth * this.m_histBarClassCounts.length - 1 : x + barWidth * this.m_histBarClassCounts.length, this.getHeight() - 16,
								(barWidth == 1) ? x + barWidth * this.m_histBarClassCounts.length - 1 : x + barWidth * this.m_histBarClassCounts.length, this.getHeight() - 12); // maximum line
						g.drawString(Utils.doubleToString(this.m_as.numericStats.max, 2), (barWidth == 1) ? x + barWidth * this.m_histBarClassCounts.length - this.m_fm.stringWidth(Utils.doubleToString(this.m_as.numericStats.max, 2)) - 1
								: x + barWidth * this.m_histBarClassCounts.length - this.m_fm.stringWidth(Utils.doubleToString(this.m_as.numericStats.max, 2)), this.getHeight() - 12 + this.m_fm.getHeight()); // maximum
						// value --
						// see 2.
						...
						g.drawLine(x, this.getHeight() - 17, (barWidth == 1) ? x + barWidth * this.m_histBarCounts.length - 1 : x + barWidth * this.m_histBarCounts.length, this.getHeight() - 17); // axis line
						g.drawLine(x, this.getHeight() - 16, x, this.getHeight() - 12); // minimum
																						// line
						g.drawString(Utils.doubleToString(this.m_as.numericStats.min, 2), x, this.getHeight() - 12 + this.m_fm.getHeight()); // minimum value
						g.drawLine(x + (barWidth * this.m_histBarCounts.length) / 2, this.getHeight() - 16, x + (barWidth * this.m_histBarCounts.length) / 2, this.getHeight() - 12); // median line
						g.drawString(Utils.doubleToString(this.m_as.numericStats.max / 2 + this.m_as.numericStats.min / 2, 2),
								x + (barWidth * this.m_histBarCounts.length) / 2 - this.m_fm.stringWidth(Utils.doubleToString(this.m_as.numericStats.max / 2 + this.m_as.numericStats.min / 2, 2)) / 2,
								this.getHeight() - 12 + this.m_fm.getHeight()); // median value
						g.drawLine((barWidth == 1) ? x + barWidth * this.m_histBarCounts.length - 1 : x + barWidth * this.m_histBarCounts.length, this.getHeight() - 16,
								(barWidth == 1) ? x + barWidth * this.m_histBarCounts.length - 1 : x + barWidth * this.m_histBarCounts.length, this.getHeight() - 12); // maximum
						// line
						g.drawString(Utils.doubleToString(this.m_as.numericStats.max, 2), (barWidth == 1) ? x + barWidth * this.m_histBarCounts.length - this.m_fm.stringWidth(Utils.doubleToString(this.m_as.numericStats.max, 2)) - 1
								: x + barWidth * this.m_histBarCounts.length - this.m_fm.stringWidth(Utils.doubleToString(this.m_as.numericStats.max, 2)), this.getHeight() - 12 + this.m_fm.getHeight()); // maximum
						// value
					}
			...
			else if (this.m_displayCurrentAttribute) { // if still calculation thread is
				// running plot
				g.clearRect(0, 0, this.getWidth(), this.getHeight());
