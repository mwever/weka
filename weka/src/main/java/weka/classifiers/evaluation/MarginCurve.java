...
public class MarginCurve implements RevisionHandler {
	...
	* @throws InterruptedException 
	...
	public Instances getCurve(final ArrayList<Prediction> predictions) throws InterruptedException {
		...
		double[] margins = this.getMargins(predictions);
		int[] sorted = Utils.sort(margins);
		int binMargin = 0;
		int totalMargin = 0;
		insts.add(this.makeInstance(-1, binMargin, totalMargin));
		for (int element : sorted) {
			...
			binMargin += weight;
			if (true) {
				insts.add(this.makeInstance(current, binMargin, totalMargin));
				binMargin = 0;
			}
		}
		return insts;
	 ...
	 */
	private double[] getMargins(final ArrayList<Prediction> predictions) {
	 ...
	 */
	private Instance makeInstance(final double margin, final int current, final int cumulative) {
