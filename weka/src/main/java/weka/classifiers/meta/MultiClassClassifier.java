...
/**
 * <!-- globalinfo-start --> A metaclassifier for handling multi-class datasets with 2-class
...
public class MultiClassClassifier extends RandomizableSingleClassifierEnhancer implements OptionHandler {
  ...
  public static final Tag[] TAGS_METHOD = { new Tag(METHOD_1_AGAINST_ALL, "1-against-all"), new Tag(METHOD_ERROR_RANDOM, "Random correction code"),
      new Tag(METHOD_ERROR_EXHAUSTIVE, "Exhaustive correction code"), new Tag(METHOD_1_AGAINST_1, "1-against-1") };
  ...
  public MultiClassClassifier() {
    ...
    this.m_Classifier = new weka.classifiers.functions.Logistic();
  }
  ...
  private abstract class Code implements Serializable, RevisionHandler {
    ...
    /**
     * Subclasses must allocate and fill these. First dimension is number of codes. Second dimension is
     * number of classes.
     */
    ...
    /**
     * Returns the indices of the values set to true for this code, using 1-based indexing (for input to
     * Range).
     *
     * @param which
     *          the index
     ...
     */
    public String getIndices(final int which) {
  ...
  private class StandardCode extends Code {
     ...
     */
    public StandardCode(final int numClasses) {
  ...
  private class RandomCode extends Code {
     ...
     *          the data to use
     */
    public RandomCode(final int numClasses, int numCodes, final Instances data) {
    ...
    private void randomize() {
      for (int i = 0; i < this.m_Codebits.length; i++) {
        for (int j = 0; j < this.m_Codebits[i].length; j++) {
          ...
          this.m_Codebits[i][j] = (temp < 0.5) ? false : true;
        }
      }
    }
  ...
  /*
   * TODO: Constructs codes as per: Bose, R.C., Ray Chaudhuri (1960), On a class of error-correcting
   * binary group codes, Information and Control, 3, 68-79. Hocquenghem, A. (1959) Codes corecteurs
   * d'erreurs, Chiffres, 2, 147-156.
   */
  ...
  private class ExhaustiveCode extends Code {
     ...
     *          the number of classes
     */
    public ExhaustiveCode(final int numClasses) {
  ...
  public void buildClassifier(Instances insts) throws Exception {
    ...
    if (numClassifiers <= 2) {
    ...
    } else if (this.m_Method == METHOD_1_AGAINST_1) {
      ...
      for (int i = 0; i < numClassifiers; i++) {
        ...
        classFilter.setNominalIndicesArr(pairs.get(i));
      ...
      switch (this.m_Method) {
        ...
        case METHOD_ERROR_RANDOM:
          code = new RandomCode(numClassifiers, (int) (numClassifiers * this.m_RandomWidthFactor), insts);
          break;
  ...
  /**
   * Returns the individual predictions of the base classifiers for an instance. Used by
   * StackedMultiClassClassifier. Returns the probability for the second "class" predicted by each
   * base classifier.
   *
   * @param inst
   ...
   */
  public double[] individualPredictions(final Instance inst) throws Exception {

    double[] result = null;

    if (this.m_Classifiers.length == 1) {
      result = new double[1];
      result[0] = this.m_Classifiers[0].distributionForInstance(inst)[1];
    } else {
      result = new double[this.m_ClassFilters.length];
      for (int i = 0; i < this.m_ClassFilters.length; i++) {
        if (this.m_Classifiers[i] != null) {
          if (this.m_Method == METHOD_1_AGAINST_1) {
  ...
  public double[] distributionForInstance(final Instance inst) throws Exception {
    ...
    } else if (this.m_Method == METHOD_1_AGAINST_ALL) {
      for (int i = 0; i < this.m_ClassFilters.length; i++) {
        // XXX kill weka execution
        if (Thread.currentThread().isInterrupted()) {
          throw new InterruptedException("Thread got interrupted, thus, kill WEKA.");
  ...
  public Enumeration<Option> listOptions() {
    ...
    Vector<Option> vec = new Vector<>(3);
  ...
  public void setOptions(final String[] options) throws Exception {
    ...
    if (errorString.length() != 0) {
      ...
      this.setMethod(new SelectedTag(METHOD_1_AGAINST_ALL, TAGS_METHOD));
    }

    String rfactorString = Utils.getOption('R', options);
    if (rfactorString.length() != 0) {
      this.setRandomWidthFactor((new Double(rfactorString)).doubleValue());
    } else {
  ...
  public String globalInfo() {
    ...
    return "A metaclassifier for handling multi-class datasets with 2-class " + "classifiers. This classifier is also capable of "
   ...
   *
   * @param newlogLossDecoding
   *          true if log loss is to be used
   */
  public void setLogLossDecoding(final boolean newlogLossDecoding) {
  ...
  /**
   * Gets the multiplier when generating random codes. Will generate numClasses * m_RandomWidthFactor
   * codes.
   *
  ...
  /**
   * Sets the multiplier when generating random codes. Will generate numClasses * m_RandomWidthFactor
   * codes.
   *
   * @param newRandomWidthFactor
   *          the new width multiplier
   */
  public void setRandomWidthFactor(final double newRandomWidthFactor) {
  ...
  /**
   * Gets the method used. Will be one of METHOD_1_AGAINST_ALL, METHOD_ERROR_RANDOM,
   * METHOD_ERROR_EXHAUSTIVE, or METHOD_1_AGAINST_1.
   *
  ...
  /**
   * Sets the method used. Will be one of METHOD_1_AGAINST_ALL, METHOD_ERROR_RANDOM,
   * METHOD_ERROR_EXHAUSTIVE, or METHOD_1_AGAINST_1.
   *
   * @param newMethod
   *          the new method.
   */
  public void setMethod(final SelectedTag newMethod) {
   ...
   */
  public void setUsePairwiseCoupling(final boolean p) {
    this.m_pairwiseCoupling = p;
  }

  /**
   * Gets whether to use pairwise coupling with 1-vs-1 classification to improve probability
   * estimates.
   *
   ...
   *
   * @param n
   *          the sum of weights used to train each model
   * @param r
   *          the probability estimate from each model
   * @return the coupled estimates
   */
  public static double[] pairwiseCoupling(final double[][] n, final double[][] r) {
