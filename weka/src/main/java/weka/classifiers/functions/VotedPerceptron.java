...
/**
 * <!-- globalinfo-start --> Implementation of the voted perceptron algorithm by Freund and
 * Schapire. Globally replaces all missing values, and transforms nominal attributes into binary
 * ones.<br/>
 * <br/>
 * For more information, see:<br/>
 * <br/>
 * Y. Freund, R. E. Schapire: Large margin classification using the perceptron algorithm. In: 11th
 * Annual Conference on Computational Learning Theory, New York, NY, 209-217, 1998.
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class VotedPerceptron extends AbstractClassifier implements OptionHandler, TechnicalInformationHandler {
  ...
  public String globalInfo() {
    return "Implementation of the voted perceptron algorithm by Freund and " + "Schapire. Globally replaces all missing values, and transforms "
  ...
  public void setOptions(final String[] options) throws Exception {
    ...
    if (exponentsString.length() != 0) {
      this.m_Exponent = (new Double(exponentsString)).doubleValue();
    } else {
      this.m_Exponent = 1.0;
    }
    ...
    if (alterationsString.length() != 0) {
      this.m_MaxK = Integer.parseInt(alterationsString);
    } else {
      this.m_MaxK = 10000;
    }
  ...
  public double[] distributionForInstance(Instance inst) throws Exception {
    ...
    if (this.m_K > 0) {
      for (int i = 0; i <= this.m_K; i++) {
        ...
        if (sumSoFar < 0) {
          output -= this.m_Weights[i];
        } else {
          output += this.m_Weights[i];
        }
        if (this.m_IsAddition[i]) {
          sumSoFar += this.innerProduct(this.m_Train.instance(this.m_Additions[i]), inst);
        } else {
          sumSoFar -= this.innerProduct(this.m_Train.instance(this.m_Additions[i]), inst);
        }
      }
   ...
   */
  public void setMaxK(final int v) {
   ...
   */
  public void setNumIterations(final int v) {
   ...
   */
  public void setExponent(final double v) {
   ...
   */
  public void setSeed(final int v) {
   ...
   */
  private double innerProduct(final Instance i1, final Instance i2) throws Exception {
   ...
   *           if computation fails
   */
  private int makePrediction(final int k, final Instance inst) throws Exception {

    double result = 0;
    for (int i = 0; i < k; i++) {
      if (this.m_IsAddition[i]) {
        result += this.innerProduct(this.m_Train.instance(this.m_Additions[i]), inst);
      } else {
        result -= this.innerProduct(this.m_Train.instance(this.m_Additions[i]), inst);
      }
    }
