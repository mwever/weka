...
/**
 * <!-- globalinfo-start --> Class for building and using a multinomial logistic regression model
 * with a ridge estimator.<br/>
 * <br/>
 ...
 * } + ridge * (B^2)<br/>
 * <br/>
 * In order to find the matrix B for which L is minimised, a Quasi-Newton Method is used to search
 * for the optimized values of the m*(k-1) variables. Note that before we use the optimization
 * procedure, we 'squeeze' the matrix B into a m*(k-1) vector. For details of the optimization
 * procedure, please check weka.core.Optimization class.<br/>
 * <br/>
 * Although original Logistic Regression does not deal with instance weights, we modify the
 * algorithm a little bit to handle the instance weights.<br/>
 * <br/>
 * For more information see:<br/>
 * <br/>
 * le Cessie, S., van Houwelingen, J.C. (1992). Ridge Estimators in Logistic Regression. Applied
 * Statistics. 41(1):191-201.<br/>
 * <br/>
 * Note: Missing values are replaced using a ReplaceMissingValuesFilter, and nominal attributes are
 * transformed into numeric attributes using a NominalToBinaryFilter.
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class Logistic extends AbstractClassifier implements OptionHandler, WeightedInstancesHandler, TechnicalInformationHandler, PMMLProducer, Aggregateable<Logistic> {
  ...
  public String globalInfo() {
    return "Class for building and using a multinomial logistic " + "regression model with a ridge estimator.\n\n"
        ...
        + "the m*(k-1) variables.  Note that before we use the optimization " + "procedure, we 'squeeze' the matrix B into a m*(k-1) vector.  For "
        ...
        + "weights, we modify the algorithm a little bit to handle the " + "instance weights.\n\n" + "For more information see:\n\n" + this.getTechnicalInformation().toString()
        + "\n\n" + "Note: Missing values are replaced using a ReplaceMissingValuesFilter, and " + "nominal attributes are transformed into numeric attributes using a "
   ...
   *          true if CGD is to be used.
   */
  public void setUseConjugateGradientDescent(final boolean useConjugateGradientDescent) {
   ...
   */
  public void setMaxIts(final int newMaxIts) {
  ...
  private class OptEng extends Optimization {
    ...
    private OptEng(final OptObject oO) {
      this.m_oO = oO;
    }
  ...
  private class OptEngCG extends ConjugateGradientOptimization {
    ...
    private OptEngCG(final OptObject oO) {
      this.m_oO = oO;
    }
  ...
  private class OptObject {
     ...
     */
    public void setClassLabels(final int[] c) {
      this.cls = c;
    }
     ...
     *
     * This is based on Tobias P. Mann's description in "Numerically Stable Hidden Markov
     * Implementation" (2006).
     */
    ...
    protected double objectiveFunction(final double[] x) {
      ...
      for (int i = 0; i < this.cls.length; i++) { // ith instance
        ...
        for (int offset = 0; offset < Logistic.this.m_NumClasses - 1; offset++) {
          index = offset * dim;
          for (int j = 0; j < dim; j++) {
            exp[offset] += Logistic.this.m_Data[i][j] * x[index + j];
          }
        }
        double num = 0;
        if (this.cls[i] < Logistic.this.m_NumClasses - 1) { // Class of this instance
          num = exp[this.cls[i]];
        }
        double denom = 0;
        for (int offset = 0; offset < Logistic.this.m_NumClasses - 1; offset++) {
          denom = this.logOfSum(denom, exp[offset]);
        }

        nll -= this.weights[i] * (num - denom); // Weighted NLL
      }
    ...
    protected double[] evaluateGradient(final double[] x) {
      ...
      for (int i = 0; i < this.cls.length; i++) { // ith instance
        double[] num = new double[Logistic.this.m_NumClasses - 1]; // numerator of
        // [-log(1+sum(exp))]'
        int index;
        ...
        for (int offset = 0; offset < Logistic.this.m_NumClasses - 1; offset++) { // Which
          // part of x
          index = offset * dim;
          firstTerm = this.weights[i] * num[offset];
          for (int q = 0; q < dim; q++) {
            grad[index + q] += firstTerm * Logistic.this.m_Data[i][q];
          }
        }

        if (this.cls[i] != Logistic.this.m_NumClasses - 1) { // Not the last class
          for (int p = 0; p < dim; p++) {
            grad[this.cls[i] * dim + p] -= this.weights[i] * Logistic.this.m_Data[i][p];
          }
        }
      }
      ...
      for (int offset = 0; offset < Logistic.this.m_NumClasses - 1; offset++) {
        for (int r = 1; r < dim; r++) {
          grad[offset * dim + r] += 2 * Logistic.this.m_Ridge * x[offset * dim + r];
        }
      }
  ...
  public void buildClassifier(Instances train) throws Exception {
    ...
    int nK = this.m_NumClasses - 1; // Only K-1 class labels needed
    int nR = this.m_NumPredictors = train.numAttributes() - 1;
    int nC = train.numInstances();

    this.m_Data = new double[nC][nR + 1]; // Data values
    ...
    this.m_Par = new double[nR + 1][nK]; // Optimized parameter values
    ...
    for (int i = 0; i < nC; i++) {
      for (int j = 0; j <= nR; j++) {
        ...
        if (xSD[j] != 0) {
          this.m_Data[i][j] = (this.m_Data[i][j] - xMean[j]) / xSD[j];
        }
      }
    }
    ...
    if (this.m_useConjugateGradientDescent) {
      opt = new OptEngCG(oO);
    } else {
      opt = new OptEng(oO);
    }
    ...
    if (this.m_MaxIts == -1) { // Search until convergence
    ...
    this.m_LL = -opt.getMinFunction(); // Log-likelihood
    ...
    for (int i = 0; i < nK; i++) {
      ...
      for (int j = 1; j <= nR; j++) {
        ...
        if (xSD[j] != 0) {
          this.m_Par[j][i] /= xSD[j];
          this.m_Par[0][i] -= this.m_Par[j][i] * xMean[j];
        }
      }
    }
  }
   ...
   */
  private double[] evaluateProbability(final double[] data) {
    double[] prob = new double[this.m_NumClasses], v = new double[this.m_NumClasses];
  ...
  /**
   * Returns the coefficients for this logistic model. The first dimension indexes the attributes, and
  ...
  public String toString() {
    ...
    for (int i = 0; i < this.m_structure.classAttribute().numValues() - 1; i++) {
      if (this.m_structure.classAttribute().value(i).length() > colWidth) {
        colWidth = this.m_structure.classAttribute().value(i).length();
      }
    }
    ...
    for (int j = 1; j <= this.m_NumPredictors; j++) {
      for (int k = 0; k < this.m_NumClasses - 1; k++) {
        if (Utils.doubleToString(this.m_Par[j][k], 8 + this.getNumDecimalPlaces(), this.getNumDecimalPlaces()).trim().length() > colWidth) {
          colWidth = Utils.doubleToString(this.m_Par[j][k], 8 + this.getNumDecimalPlaces(), this.getNumDecimalPlaces()).trim().length();
        }
        double ORc = Math.exp(this.m_Par[j][k]);
        String t = " " + ((ORc > 1e10) ? "" + ORc : Utils.doubleToString(ORc, 8 + this.getNumDecimalPlaces(), this.getNumDecimalPlaces()));
    ...
    int j = 1;
    for (int i = 0; i < this.m_structure.numAttributes(); i++) {
      if (i != this.m_structure.classIndex()) {
        temp.append(Utils.padRight(this.m_structure.attribute(i).name(), attLength));
        for (int k = 0; k < this.m_NumClasses - 1; k++) {
          temp.append(Utils.padLeft(Utils.doubleToString(this.m_Par[j][k], 8 + this.getNumDecimalPlaces(), this.getNumDecimalPlaces()).trim(), colWidth));
        }
        temp.append("\n");
        j++;
      }
    ...
    for (int k = 0; k < this.m_NumClasses - 1; k++) {
      temp.append(Utils.padLeft(Utils.doubleToString(this.m_Par[0][k], 6 + this.getNumDecimalPlaces(), this.getNumDecimalPlaces()).trim(), colWidth));
    }
    temp.append("\n");
    ...
    for (int i = 0; i < this.m_structure.numAttributes(); i++) {
      if (i != this.m_structure.classIndex()) {
        ...
        for (int k = 0; k < this.m_NumClasses - 1; k++) {
          double ORc = Math.exp(this.m_Par[j][k]);
          String ORs = " " + ((ORc > 1e10) ? "" + ORc : Utils.doubleToString(ORc, 8 + this.getNumDecimalPlaces(), this.getNumDecimalPlaces()));
   ...
   */
  @Override
  public Logistic aggregate(final Logistic toAggregate) throws Exception {
    ...
    for (int i = 0; i < this.m_Par.length; i++) {
      for (int j = 0; j < this.m_Par[i].length; j++) {
        this.m_Par[i][j] += toAggregate.m_Par[i][j];
      }
    }
  ...
  /**
   * Call to complete the aggregation process. Allows implementers to do any final processing based on
   * how many objects were aggregated.
   *
   * @throws Exception
  ...
  public void finalizeAggregation() throws Exception {
    ...
    for (int i = 0; i < this.m_Par.length; i++) {
      for (int j = 0; j < this.m_Par[i].length; j++) {
        this.m_Par[i][j] /= (this.m_numModels + 1);
      }
    }
   ...
   */
  @Override
  public String toPMML(final Instances train) {
    return LogisticProducerHelper.toPMML(train, this.m_structure, this.m_Par, this.m_NumClasses);
  }
}
