...
public abstract class CachedKernel extends Kernel {
	...
	/**
	 * Initializes the kernel cache. The actual size of the cache in bytes is (64 * cacheSize).
	 *
	 * @param data
	 *            the data to use
	 * @param cacheSize
	 ...
	 */
	protected CachedKernel(final Instances data, final int cacheSize) throws Exception {
		super();
	 ...
	 *             if something goes wrong
	 */
	protected abstract double evaluate(int id1, int id2, Instance inst1) throws Exception;
	 ...
	 *
	 * @param id1
	 *            the index of the first instance in the dataset
	 * @param id2
	 ...
	 */
	@Override
	public double eval(final int id1, final int id2, final Instance inst1) throws Exception {
		...
		if ((id1 >= 0) && (this.m_cacheSize != -1)) {
			...
			// Use LRU cache
			if (id1 > id2) {
				key = (id1 + ((long) id2 * this.m_numInsts));
			} else {
				key = (id2 + ((long) id1 * this.m_numInsts));
			}
			location = (int) (key % this.m_cacheSize) * this.m_cacheSlots;
			int loc = location;
			for (int i = 0; i < this.m_cacheSlots; i++) {
	 ...
	 *             if an error occurs
	 */
	protected final double dotProd(final Instance inst1, final Instance inst2) throws Exception {
		double result = 0;
	 ...
	 */
	@Override
	protected void initVars(final Instances data) {
		super.initVars(data);
	 ...
	 */
	@Override
	public void buildKernel(final Instances data) throws Exception {
