...
public class RBFKernel extends CachedKernel {
	 ...
	 */
	public RBFKernel(final Instances data, final int cacheSize, final double gamma) throws Exception {
	 ...
	 */
	@Override
	protected double evaluate(final int id1, final int id2, final Instance inst1) throws Exception {

		if (id1 == id2) {
			return 1.0;
		} else {
			if (id1 == -1) {
				return Math.exp(-this.m_gamma * (this.dotProd(inst1, inst1) - 2 * this.dotProd(inst1, this.m_data.instance(id2)) + this.m_kernelPrecalc[id2]));
			} else {
				return Math.exp(-this.m_gamma * (this.m_kernelPrecalc[id1] - 2 * this.dotProd(inst1, this.m_data.instance(id2)) + this.m_kernelPrecalc[id2]));
			}
		}
	}
