...
public class Puk extends CachedKernel implements TechnicalInformationHandler {
	 ...
	 */
	public Puk(final Instances data, final int cacheSize, final double omega, final double sigma) throws Exception {
	...
	protected double evaluate(final int id1, final int id2, final Instance inst1) throws Exception {
		...
		if (id1 == id2) {
			...
			double precalc1;
			if (id1 == -1) {
				precalc1 = this.dotProd(inst1, inst1);
			} else {
				precalc1 = this.m_kernelPrecalc[id1];
			}
	 ...
	 *            the sigma value
	 */
	public void setSigma(final double value) {
		this.m_sigma = value;
		this.m_factor = this.computeFactor(this.m_omega, this.m_sigma);
	}
	 ...
	 */
	protected double computeFactor(final double omega, final double sigma) {
