...
/**
 * <!-- globalinfo-start --> This Bayes Network learning algorithm uses genetic search for finding a well scoring Bayes network structure. Genetic search works by having a population of Bayes network structures and allow them to mutate and
 * apply cross over to get offspring. The best network structure found during the process is returned.
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class GeneticSearch extends LocalScoreSearchAlgorithm {
	...
	class BayesNetRepresentation implements RevisionHandler {
		...
		/**
		 * bit representation of parent sets m_bits[iTail + iHead * m_nNodes] represents arc iTail->iHead
		 */
		boolean[] m_bits;
		 ...
		 *            the number of nodes
		 */
		BayesNetRepresentation(final int nNodes) {
		 ...
		 */
		public void randomInit() throws InterruptedException {
			do {
				this.m_bits = new boolean[this.m_nNodes * this.m_nNodes];
				for (int i = 0; i < this.m_nNodes; i++) {
					int iPos;
					do {
						iPos = GeneticSearch.this.m_random.nextInt(this.m_nNodes * this.m_nNodes);
					} while (this.isSquare(iPos));
		 ...
		 * @throws InterruptedException
		 */
		void calcScore() throws InterruptedException {
			// clear current network
			for (int iNode = 0; iNode < this.m_nNodes; iNode++) {
				ParentSet parentSet = GeneticSearch.this.m_BayesNet.getParentSet(iNode);
		 ...
		 * @throws InterruptedException
		 */
		void mutate() throws InterruptedException {
			// flip a bit
			do {
				int iBit;
				do {
					iBit = GeneticSearch.this.m_random.nextInt(this.m_nNodes * this.m_nNodes);
				} while (this.isSquare(iBit));

				this.m_bits[iBit] = !this.m_bits[iBit];
			} while (this.hasCycles());
		 ...
		 * @throws InterruptedException
		 */
		void crossOver(final BayesNetRepresentation other) throws InterruptedException {
			...
			do {
				...
				for (int i = iCrossOverPoint; i < this.m_bits.length; i++) {
					this.m_bits[i] = bits[i];
				}
				// take all bits from cross-over points onwards
				iCrossOverPoint = GeneticSearch.this.m_random.nextInt(this.m_bits.length);
				for (int i = iCrossOverPoint; i < this.m_bits.length; i++) {
		 ...
		 */
		boolean isSquare(final int nNum) {
			if (GeneticSearch.this.g_bIsSquare == null || GeneticSearch.this.g_bIsSquare.length < nNum) {
				...
				for (int i = 0; i < this.m_nNodes; i++) {
					GeneticSearch.this.g_bIsSquare[i * this.m_nNodes + i] = true;
				}
			}
	 ...
	 *            the data to use
	 * @throws Exception
	 *             if population size doesn fit or neither cross-over or mutation was chosen
	 */
	@Override
	 ...
	 */
	public void setUseCrossOver(final boolean bUseCrossOver) {
	 ...
	 */
	public void setUseMutation(final boolean bUseMutation) {
	 ...
	 */
	public void setUseTournamentSelection(final boolean bUseTournamentSelection) {
	 ...
	 */
	public void setDescendantPopulationSize(final int iDescendantPopulationSize) {
	 ...
	 */
	public void setPopulationSize(final int iPopulationSize) {
	...
	public String globalInfo() {
		return "This Bayes Network learning algorithm uses genetic search for finding a well scoring " + "Bayes network structure. Genetic search works by having a population of Bayes network structures "
				+ "and allow them to mutate and apply cross over to get offspring. The best network structure " + "found during the process is returned.";
	} // globalInfo
	...
	public String useCrossOverTipText() {
		return "Determines whether cross-over is allowed. Cross over combined the bit " + "representations of network structure by taking a random first k bits of one"
				+ "and adding the remainder of the other. At least one of mutation or cross-over " + "should be used.";
	} // useCrossOverTipText
	...
	public String useTournamentSelectionTipText() {
		return "Determines the method of selecting a population. When set to true, tournament " + "selection is used (pick two at random and the highest is allowed to continue). "
