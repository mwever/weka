...
public class SearchAlgorithm implements OptionHandler, Serializable, RevisionHandler {
	 ...
	 */
	protected boolean addArcMakesSense(final BayesNet bayesNet, final Instances instances, final int iAttributeHead, final int iAttributeTail) throws InterruptedException {
	 ...
	 */
	protected boolean reverseArcMakesSense(final BayesNet bayesNet, final Instances instances, final int iAttributeHead, final int iAttributeTail) {
	 ...
	 *         exists
	 */
	protected boolean isArc(final BayesNet bayesNet, final int iAttributeHead, final int iAttributeTail) {
	...
	public void buildStructure(final BayesNet bayesNet, final Instances instances) throws Exception {
		if (this.m_sInitalBIFFile != null && !this.m_sInitalBIFFile.equals("")) {
			BIFReader initialNet = new BIFReader().processFile(this.m_sInitalBIFFile);
	 ...
	 * @param instances
	 */
	protected void doMarkovBlanketCorrection(final BayesNet bayesNet, final Instances instances) {
	 ...
	 */
	protected void setMarkovBlanketClassifier(final boolean bMarkovBlanketClassifier) {
	...
	public String maxNrOfParentsTipText() {
		return "Set the maximum number of parents a node in the Bayes net can have." + " When initialized as Naive Bayes, setting this parameter to 1 results in"
				+ " a Naive Bayes classifier. When set to 2, a Tree Augmented Bayes Network (TAN)" + " is learned, and when set >2, a Bayes Net Augmented Bayes Network (BAN)"
				+ " is learned. By setting it to a value much larger than the number of nodes" + " in the network (the default of 100000 pretty much guarantees this), no" + " restriction on the number of parents is enforced";
	...
	public String initAsNaiveBayesTipText() {
		return "When set to true (default), the initial network used for structure learning" + " is a Naive Bayes Network, that is, a network with an arrow from the classifier"
				+ " node to each other node. When set to false, an empty network is used as initial" + " network structure";
	...
	protected String markovBlanketClassifierTipText() {
		return "When set to true (default is false), after a network structure is learned" + " a Markov Blanket correction is applied to the network structure. This ensures"
				+ " that all nodes in the network are part of the Markov blanket of the classifier" + " node.";
