...
/**
 * <!-- globalinfo-start --> This Bayes Network learning algorithm uses the general purpose search method of simulated annealing to find a well scoring network structure.<br/>
 * <br/>
...
public class SimulatedAnnealing extends LocalScoreSearchAlgorithm implements TechnicalInformationHandler {
	 ...
	 */
	@Override
	public void search(final BayesNet bayesNet, final Instances instances) throws Exception {
		...
		for (int iRun = 0; iRun < this.m_nRuns; iRun++) {
			...
			while (!bRunSucces) {
				...
				int iTailNode = this.m_random.nextInt(instances.numAttributes());
				int iHeadNode = this.m_random.nextInt(instances.numAttributes());
				while (iTailNode == iHeadNode) {
					iHeadNode = this.m_random.nextInt(instances.numAttributes());
				}
				if (this.isArc(bayesNet, iHeadNode, iTailNode)) {
					bRunSucces = true;
					// either try a delete
					bayesNet.getParentSet(iHeadNode).deleteParent(iTailNode, instances);
					double fScore = this.calcNodeScore(iHeadNode);
					fDeltaScore = fScore - fBaseScores[iHeadNode];
					// System.out.println("Try delete " + iTailNode + "->" + iHeadNode +
					// " dScore = " + fDeltaScore);
					if (fTemp * Math.log((Math.abs(this.m_random.nextInt()) % 10000) / 10000.0 + 1e-100) < fDeltaScore) {
					...
					// try to add an arc
					if (this.addArcMakesSense(bayesNet, instances, iHeadNode, iTailNode)) {
						bRunSucces = true;
						double fScore = this.calcScoreWithExtraParent(iHeadNode, iTailNode);
						fDeltaScore = fScore - fBaseScores[iHeadNode];
	 ...
	 */
	void copyParentSets(final BayesNet dest, final BayesNet source) throws InterruptedException {
	 ...
	 *            The m_fDelta to set
	 */
	public void setDelta(final double fDelta) {
	 ...
	 */
	public void setTStart(final double fTStart) {
	...
	public String TStartTipText() {
		return "Sets the start temperature of the simulated annealing search. " + "The start temperature determines the probability that a step in the 'wrong' direction in the "
