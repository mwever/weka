...
/**
 * <!-- globalinfo-start --> This Bayes Network learning algorithm uses conditional independence tests to find a skeleton, finds V-nodes and applies a set of rules to find the directions of the remaining arrows.
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class ICSSearchAlgorithm extends CISearchAlgorithm {
	...
	void calcDependencyGraph(final boolean[][] edges, final SeparationSet[][] sepsets) {
		...
		for (int iCardinality = 0; iCardinality <= this.getMaxCardinality(); iCardinality++) {
			for (int iNode1 = 0; iNode1 <= this.maxn() - 2; iNode1++) {
				for (int iNode2 = iNode1 + 1; iNode2 < this.maxn(); iNode2++) {
					if (edges[iNode1][iNode2]) {
						oSepSet = this.existsSepSet(iNode1, iNode2, iCardinality, edges);
						if (oSepSet != null) {
							...
							System.err.print("I(" + this.name(iNode1) + ", {");
			...
			for (int iNode1 = 0; iNode1 < this.maxn(); iNode1++) {
				System.err.print(this.name(iNode1) + " ");
			}
			System.err.print('\n');
			for (int iNode1 = 0; iNode1 < this.maxn(); iNode1++) {
				for (int iNode2 = 0; iNode2 < this.maxn(); iNode2++) {
	 ...
	 *            first node
	 * @param iNode2
	 *            second node (must be larger than iNode1)
	 * @param edges
	 *            skeleton so far
	 ...
	 *            skeleton
	 * @param arrows
	 *            resulting partially directed skeleton after all V-nodes have been identified
	 * @param sepsets
	 *            separating sets
	 */
	...
	public String maxCardinalityTipText() {
		return "When determining whether an edge exists a search is performed for a set Z " + "that separates the nodes. MaxCardinality determines the maximum size of the set Z. "
