...
/**
 * <!-- globalinfo-start --> This Bayes Network learning algorithm uses a hill climbing algorithm restricted by an order on the variables.<br/>
 * <br/>
...
public class K2 extends LocalScoreSearchAlgorithm implements TechnicalInformationHandler {
	...
	public void search(final BayesNet bayesNet, final Instances instances) throws Exception {
		...
		for (int iOrder = 1; iOrder < instances.numAttributes(); iOrder++) {
			...
			boolean bProgress = (bayesNet.getParentSet(iAttribute).getNrOfParents() < this.getMaxNrOfParents());
			while (bProgress) {
				...
				if (nBestAttribute != -1) {
					...
					bProgress = (bayesNet.getParentSet(iAttribute).getNrOfParents() < this.getMaxNrOfParents());
				} else {
					bProgress = false;
				}
			}
	 ...
	 */
	public void setRandomOrder(final boolean bRandomOrder) {
	...
	public String randomOrderTipText() {
		return "When set to true, the order of the nodes in the network is random." + " Default random order is false and the order" + " of the nodes in the dataset is used."
				+ " In any case, when the network was initialized as Naive Bayes Network, the" + " class variable is first in the ordering though.";
	} // randomOrderTipText
