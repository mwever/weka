...
/**
 * <!-- globalinfo-start --> This Bayes Network learning algorithm uses tabu search for finding a well scoring Bayes network structure. Tabu search is hill climbing till an optimum is reached. The following step is the least worst possible
 * step. The last X steps are kept in a list and none of the steps in this so called tabu list is considered in taking the next step. The best network found in this traversal is returned.<br/>
 * <br/>
...
public class TabuSearch extends HillClimber implements TechnicalInformationHandler {
	...
	protected void search(final BayesNet bayesNet, final Instances instances) throws Exception {
		this.m_oTabuList = new Operation[this.m_nTabuList];
	...
	boolean isNotTabu(final Operation oOperation) {
		for (int iTabu = 0; iTabu < this.m_nTabuList; iTabu++) {
			if (oOperation.equals(this.m_oTabuList[iTabu])) {
				return false;
			}
		}
		return true;
	...
	void printTabuList() {
		for (int i = 0; i < this.m_nTabuList; i++) {
			Operation o = this.m_oTabuList[i];
			if (o != null) {
				if (o.m_nOperation == 0) {
					System.out.print(" +(");
				} else {
	 ...
	 */
	public void setTabuList(final int nTabuList) {
	...
	public String globalInfo() {
		return "This Bayes Network learning algorithm uses tabu search for finding a well scoring " + "Bayes network structure. Tabu search is hill climbing till an optimum is reached. The "
				+ "following step is the least worst possible step. The last X steps are kept in a list and " + "none of the steps in this so called tabu list is considered in taking the next step. "
