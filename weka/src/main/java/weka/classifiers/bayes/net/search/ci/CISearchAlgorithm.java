...
/**
 * <!-- globalinfo-start --> The CISearchAlgorithm class supports Bayes net structure search algorithms that are based on conditional independence test (as opposed to for example score based of cross validation based search algorithms).
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class CISearchAlgorithm extends LocalScoreSearchAlgorithm {
	 ...
	 * 
	 * @param iAttributeX
	 *            - index of attribute representing variable X
	 * @param iAttributeY
	 *            - index of attribute representing variable Y
	 * @param iAttributesZ
	 *            - array of integers representing indices of attributes in set Z
	 * @param nAttributesZ
	 *            - cardinality of Z
	 * @return true if X and Y conditionally independent given Z
	 * @throws InterruptedException
	 */
	protected boolean isConditionalIndependent(final int iAttributeX, final int iAttributeY, final int[] iAttributesZ, final int nAttributesZ) throws InterruptedException {
		ParentSet oParentSetX = this.m_BayesNet.getParentSet(iAttributeX);
		// clear parent set of AttributeX
		while (oParentSetX.getNrOfParents() > 0) {
			oParentSetX.deleteLastParent(this.m_instances);
		}

		// insert parents in iAttributeZ
		for (int iAttributeZ = 0; iAttributeZ < nAttributesZ; iAttributeZ++) {
			oParentSetX.addParent(iAttributesZ[iAttributeZ], this.m_instances);
		}

		double fScoreZ = this.calcNodeScore(iAttributeX);
		double fScoreZY = this.calcScoreWithExtraParent(iAttributeX, iAttributeY);
		if (fScoreZY <= fScoreZ) {
