...
/**
 * <!-- globalinfo-start --> This Bayes Network learning algorithm uses a Look Ahead Hill Climbing algorithm called LAGD Hill Climbing. Unlike Greedy Hill Climbing it doesn't calculate a best greedy operation (adding, deleting or reversing
 * an arc) but a sequence of nrOfLookAheadSteps operations, which leads to a network structure whose score is most likely higher in comparison to the network obtained by performing a sequence of nrOfLookAheadSteps greedy operations. The
 * search is not restricted by an order on the variables (unlike K2). The difference with B and B2 is that this hill climber also considers arrows part of the naive Bayes structure for deletion.
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class LAGDHillClimber extends HillClimber {
	...
	protected void search(final BayesNet bayesNet, final Instances instances) throws Exception {
		int k = this.m_nNrOfLookAheadSteps; // Number of Look Ahead Steps
		int l = this.m_nNrOfGoodOperations; // Number of Good Operations per step
	 ...
	 *             if something goes wrong
	 */
	protected void lookAheadInGoodDirectionsSearch(final BayesNet bayesNet, final Instances instances, int nrOfLookAheadSteps, final int nrOfGoodOperations) throws Exception {
	 ...
	 *             if something goes wrong
	 */
	protected Operation getAntiOperation(final Operation oOperation) throws Exception {
	 ...
	 *             if something goes wrong
	 **/
	protected Operation[] getGoodOperations(final BayesNet bayesNet, final Instances instances, final int nrOfGoodOperations) throws Exception {
		...
		for (int i = 0; i < nrOfGoodOperations; i++) {
			...
			if (goodOperations[i] != null) {
				this.m_Cache.put(goodOperations[i], -1E100);
			} else {
	 ...
	 *            data set to learn from
	 * @param nrOfLookAheadSteps
	 *            number of lood ahead steps to use
	 * @param nrOfGoodOperations
	 *            number of good operations to consider
	 ...
	 *             if something goes wrong
	 **/
	protected Operation[] getOptimalOperations(final BayesNet bayesNet, final Instances instances, final int nrOfLookAheadSteps, final int nrOfGoodOperations) throws Exception {
		if (nrOfLookAheadSteps == 1) { // Abbruch der Rekursion
			Operation[] bestOperation = new Operation[1];
			bestOperation[0] = this.getOptimalOperation(bayesNet, instances);
			...
			for (int i = 0; i < nrOfGoodOperations; i++) {
				...
				if (goodOperations[i] != null) {
					...
					tempOperation = this.getOptimalOperations(bayesNet, instances, nrOfLookAheadSteps - 1, nrOfGoodOperations); // rekursiver Abstieg
	 ...
	 */
	public void setNrOfLookAheadSteps(final int nNrOfLookAheadSteps) {
	 ...
	 */
	public void setNrOfGoodOperations(final int nNrOfGoodOperations) {
	...
	public String[] getOptions() {
		...
		options.add("-L");
		options.add("" + this.m_nNrOfLookAheadSteps);

		options.add("-G");
		options.add("" + this.m_nNrOfGoodOperations);
	...
	public String globalInfo() {
		return "This Bayes Network learning algorithm uses a Look Ahead Hill Climbing algorithm called LAGD Hill Climbing."
				...
				+ "higher in comparison to the network obtained by performing a sequence of nrOfLookAheadSteps greedy operations. " + "The search is not restricted by an order "
	...
	public String nrOfLookAheadStepsTipText() {
		return "Sets the Number of Look Ahead Steps. 'nrOfLookAheadSteps = 2' means that all network structures in a " + "distance of 2 (from the current network structure) are taken into account for the decision which arcs to add, "
	...
	public String nrOfGoodOperationsTipText() {
		return "Sets the Number of Good Operations per Look Ahead Step. 'nrOfGoodOperations = 5' means that for the next " + "Look Ahead Step only the 5 best Operations (adding, deleting or reversing an arc) are taken into account for the "
