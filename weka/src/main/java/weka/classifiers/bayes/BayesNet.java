...
/**
 * <!-- globalinfo-start --> Bayes Network learning using various search algorithms and quality measures.<br/>
...
public class BayesNet extends AbstractClassifier implements OptionHandler, WeightedInstancesHandler, Drawable, AdditionalMeasureProducer {
	...
	protected Instances normalizeDataSet(Instances instances) throws Exception {
		...
		if ((this.m_nNonDiscreteAttribute > -1) && (instances.attribute(this.m_nNonDiscreteAttribute).type() != Attribute.NOMINAL)) {
	...
	protected Instance normalizeInstance(Instance instance) throws Exception {
		if ((this.m_nNonDiscreteAttribute > -1) && (instance.attribute(this.m_nNonDiscreteAttribute).type() != Attribute.NOMINAL)) {
	...
	/**
	 * buildStructure determines the network structure/graph of the network. The default behavior is creating a network where all nodes have the first node as its parent (i.e., a BayesNet that behaves like a naive Bayes classifier). This
	 * method can be overridden by derived classes to restrict the class of network structures that are acceptable.
	 * 
	 * @throws Exception
	 *             in case of an error
	 */
	 ...
	 *             if there is a problem generating the prediction
	 */
	public double[] countsForInstance(final Instance instance) throws Exception {
		double[] fCounts = new double[this.m_NumClasses];

		for (int iClass = 0; iClass < this.m_NumClasses; iClass++) {
			fCounts[iClass] = 0.0;
		}
		...
		for (int iClass = 0; iClass < this.m_NumClasses; iClass++) {
			...
			for (int iAttribute = 0; iAttribute < this.m_Instances.numAttributes(); iAttribute++) {
				...
				for (int iParent = 0; iParent < this.m_ParentSets[iAttribute].getNrOfParents(); iParent++) {
					...
					if (nParent == this.m_Instances.classIndex()) {
						iCPT = iCPT * this.m_NumClasses + iClass;
					} else {
						iCPT = iCPT * this.m_Instances.attribute(nParent).numValues() + instance.value(nParent);
					}
				}
	...
	public Enumeration<Option> listOptions() {
		...
		newVector.addElement(new Option("", "", 0, "\nOptions specific to search method " + this.getSearchAlgorithm().getClass().getName() + ":"));
		newVector.addAll(Collections.list(this.getSearchAlgorithm().listOptions()));

		newVector.addElement(new Option("", "", 0, "\nOptions specific to estimator method " + this.getEstimator().getClass().getName() + ":"));
		newVector.addAll(Collections.list(this.getEstimator().listOptions()));
	...
	public void setOptions(final String[] options) throws Exception {
		...
		if (searchAlgorithmName.length() != 0) {
			this.setSearchAlgorithm((SearchAlgorithm) Utils.forName(SearchAlgorithm.class, searchAlgorithmName, partitionOptions(options)));
		} else {
	...
	/**
	 * Returns the secondary set of options (if any) contained in the supplied options array. The secondary set is defined to be any options after the first "--" but before the "-E". These options are removed from the original options
	 * array.
	 * 
	 * @param options
	...
	public String[] getOptions() {
		...
		if (this.m_otherBayesNet != null) {
			options.add("-B");
			options.add(this.m_otherBayesNet.getFileName());
		}

		options.add("-Q");
		options.add("" + this.getSearchAlgorithm().getClass().getName());
		options.add("--");
		Collections.addAll(options, this.getSearchAlgorithm().getOptions());

		options.add("-E");
		options.add("" + this.getEstimator().getClass().getName());
		options.add("--");
		Collections.addAll(options, this.getEstimator().getOptions());
	 ...
	 * 
	 * @param newSearchAlgorithm
	 *            the SearchAlgorithm to use.
	 */
	public void setSearchAlgorithm(final SearchAlgorithm newSearchAlgorithm) {
	 ...
	 * 
	 * @param newBayesNetEstimator
	 *            the Estimator to use.
	 */
	public void setEstimator(final BayesNetEstimator newBayesNetEstimator) {
	 ...
	 */
	public void setUseADTree(final boolean bUseADTree) {
	 ...
	 */
	public void setBIFFile(final String sBIFFile) {
		try {
	...
	public String toXMLBIF03() {
		...
		for (int iAttribute = 0; iAttribute < this.m_Instances.numAttributes(); iAttribute++) {
			...
			for (int iValue = 0; iValue < this.m_Instances.attribute(iAttribute).numValues(); iValue++) {
				text.append("<OUTCOME>" + this.XMLNormalize(Utils.quote(this.m_Instances.attribute(iAttribute).value(iValue))) + "</OUTCOME>\n");
			}
		...
		for (int iAttribute = 0; iAttribute < this.m_Instances.numAttributes(); iAttribute++) {
			...
			for (int iParent = 0; iParent < this.m_ParentSets[iAttribute].getNrOfParents(); iParent++) {
				text.append("<GIVEN>" + this.XMLNormalize(Utils.quote(this.m_Instances.attribute(this.m_ParentSets[iAttribute].getParent(iParent)).name())) + "</GIVEN>\n");
			}
			text.append("<TABLE>\n");
	 ...
	 */
	protected String XMLNormalize(final String sStr) {
	...
	public String useADTreeTipText() {
		return "When ADTree (the data structure for increasing speed on counts," + " not to be confused with the classifier under the same name) is used" + " learning time goes down typically. However, because ADTrees are memory"
				+ " intensive, memory problems may occur. Switching this option off makes" + " the structure learning algorithms slower, and run with less memory." + " By default, ADTrees are used.";
	}
	...
	public String globalInfo() {
		return "Bayes Network learning using various search algorithms and " + "quality measures.\n" + "Base class for a Bayes Network classifier. Provides " + "datastructures (network structure, conditional probability "
				+ "distributions, etc.) and facilities common to Bayes Network " + "learning algorithms like K2 and B.\n\n" + "For more information see:\n\n" + "http://www.cs.waikato.ac.nz/~remco/weka.pdf";
	}
	 ...
	 */
	public String getNodeName(final int iNode) {
	 ...
	 */
	public int getCardinality(final int iNode) {
	 ...
	 * @return cardinality of the specified node
	 */
	public String getNodeValue(final int iNode, final int iValue) {
	 ...
	 */
	public int getNrOfParents(final int iNode) {
	 ...
	 */
	public int getParent(final int iNode, final int iParent) {
	 ...
	 */
	public int getParentCardinality(final int iNode) {
	 ...
	 *            index of the node
	 * @param iParent
	 *            index of the parent set, 0 <= iParent <= getParentCardinality(iNode)
	 * @param iValue
	 *            index of the value, 0 <= iValue <= getCardinality(iNode)
	 * @return probability
	 */
	public double getProbability(final int iNode, final int iParent, final int iValue) {
	 ...
	 */
	public ParentSet getParentSet(final int iNode) {
	 ...
	 */
	@Override
	public double getMeasure(final String measureName) {
