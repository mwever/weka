...
public class RandomForest extends Bagging {
	 ...
	 */
	public void setNumFeatures(final int newNumFeatures) {
	 ...
	 *          scores
	 */
	public void setComputeAttributeImportance(final boolean computeAttributeImportance) {
	 ...
	 */
	@Override
	public void setNumDecimalPlaces(final int num) {
			...
			try {
				...
				for (int i = sortedIndices.length - 1; i >= 0; i--) {
					...
					if (index != this.m_data.classIndex()) {
						buffer.append(Utils.doubleToString(impurityScores[index], 10, this.getNumDecimalPlaces())).append(" (").append(Utils.doubleToString(nodeCounts[index], 6, 0)).append(")  ").append(this.m_data.attribute(index).name())
								.append("\n");
					}
				}
