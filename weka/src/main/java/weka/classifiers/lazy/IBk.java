...
/**
 * <!-- globalinfo-start --> K-nearest neighbours classifier. Can select appropriate value of K
 * based on cross-validation. Can also do distance weighting.<br/>
 * <br/>
...
public class IBk extends AbstractClassifier implements OptionHandler, UpdateableClassifier, WeightedInstancesHandler, TechnicalInformationHandler, AdditionalMeasureProducer {
  ...
  /**
   * The value of kNN provided by the user. This may differ from m_kNN if cross-validation is being
   * used.
   */
  protected int m_kNNUpper;

  /**
   * Whether the value of k selected by cross validation has been invalidated by a change in the
   * training instances.
   */
  protected boolean m_kNNValid;

  /**
   * The maximum number of training instances allowed. When this limit is reached, old training
   * instances are removed, so the training data is "windowed". Set to 0 for unlimited numbers of
   * instances.
   */
  ...
  /**
   * Whether to minimise mean squared error rather than mean absolute error when cross-validating on
  ...
  /**
   * IBk classifier. Simple instance-based learner that uses the class of the nearest k training
   * instances for the class of the test instances.
   *
   * @param k
  ...
  /**
   * IB1 classifer. Instance-based learner. Predicts the class of the single nearest training instance
  ...
  public String windowSizeTipText() {
    return "Gets the maximum number of instances allowed in the training " + "pool. The addition of new instances above this value will result "
        + "in old instances being removed. A value of 0 signifies no limit " + "to the number of training instances.";
  }

  /**
   * Gets the maximum number of instances allowed in the training pool. The addition of new instances
   * above this value will result in old instances being removed. A value of 0 signifies no limit to
  ...
  /**
   * Sets the maximum number of instances allowed in the training pool. The addition of new instances
   * above this value will result in old instances being removed. A value of 0 signifies no limit to
   * the number of training instances.
   *
   * @param newWindowSize
  ...
  /**
   * Gets whether the mean squared error is used rather than mean absolute error when doing
   * cross-validation.
   *
   * @return true if so.
   */
  ...
  /**
   * Sets whether the mean squared error is used rather than mean absolute error when doing
   * cross-validation.
   *
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tMaximum number of training instances maintained.\n" + "\tTraining instances are dropped FIFO. (Default = no window)", "W", 1, "-W <window size>"));
    newVector.addElement(new Option(
        "\tSelect the number of nearest neighbours between 1\n" + "\tand the k value specified using hold-one-out evaluation\n" + "\ton the training data (use when k > 1)", "X", 0,
        "-X"));
  ...
  /**
   * Returns the value of the named measure from the neighbour search algorithm, plus the chosen K in
   * case cross-validation is enabled.
   *
  ...
  /**
   * Select the best value for k by hold-one-out cross-validation. If the class attribute is nominal,
   * classification error is minimised. If the class attribute is numeric, mean absolute error is
   * minimised
   */
  ...
  /**
   * Prunes the list to contain the k nearest neighbors. If there are multiple neighbors at the k'th
   * distance, all will be kept.
   *
   * @param neighbours
