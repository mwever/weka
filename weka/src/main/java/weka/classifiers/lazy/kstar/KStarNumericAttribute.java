...
public class KStarNumericAttribute implements KStarConstants, RevisionHandler {
	 ...
	 * Constructor
	 */
	public KStarNumericAttribute(final Instance test, final Instance train, final int attrIndex, final Instances trainSet, final int[][] randClassCols, final KStarCache cache) {
		this.m_Test = test;
		this.m_Train = train;
	...
	public double transProb() throws Exception {
		...
		if (this.m_Cache.containsKey(this.m_Test.value(this.m_AttrIndex))) {
			...
			if (this.m_BlendMethod == B_ENTROPY) {
				this.m_Scale = this.scaleFactorUsingEntropy();
			} else { // default is B_SPHERE
				this.m_Scale = this.scaleFactorUsingBlend();
			}
		...
		if (this.m_Train.isMissing(this.m_AttrIndex)) {
			...
			distance = Math.abs(this.m_Test.value(this.m_AttrIndex) - this.m_Train.value(this.m_AttrIndex));
	...
	private double scaleFactorUsingBlend() {
		...
		for (j = 0; j < this.m_NumInstances; j++) {
			if (this.m_TrainSet.instance(j).isMissing(this.m_AttrIndex)) {
				...
				if ((this.m_Distances[j] + 1e-5) < nextlowest || nextlowest == -1.0) {
					if ((this.m_Distances[j] + 1e-5) < lowest || lowest == -1.0) {
						nextlowest = lowest;
						lowest = this.m_Distances[j];
						lowestcount = 1;
					} else if (Math.abs(this.m_Distances[j] - lowest) < 1e-5) {
		...
		if (nextlowest == -1 || lowest == -1) { // Data values are all the same
			scale = 1.0;
			this.m_SmallestProb = this.m_AverageProb = 1.0;
			return scale;
		} else {
			...
			// aimfor = (N - n0) * b + n0
			aimfor = (this.m_ActualCount - lowestcount) * (double) this.m_BlendFactor / 100.0 + lowestcount;
	 ...
	 */
	private void calculateSphereSize(final double scale, final KStarWrapper params) {
		int i;
		...
		for (i = 0; i < this.m_NumInstances; i++) {
			if (this.m_Distances[i] < 0) {
				// instance with missing value
				continue;
			} else {
				pstar = this.PStar(this.m_Distances[i], scale);
				if (minprob > pstar) {
					minprob = pstar;
				}
				inc = pstar / this.m_ActualCount;
				pstarSum += inc;
	 ...
	 * @throws Exception
	 */
	private double scaleFactorUsingEntropy() throws Exception {
	 ...
	 */
	private void calculateEntropy(final double scale, final KStarWrapper params) {
		int i, j, k;
		double actent = 0.0, randent = 0.0;
		double pstar, tprob, avgprob = 0.0, minprob = 1.0;
		double actClassProb, randClassProb;
		double[][] pseudoClassProbs = new double[NUM_RAND_COLS + 1][this.m_NumClasses];
		// init
		...
		for (i = 0; i < this.m_NumInstances; i++) {
			if (this.m_Distances[i] < 0) {
				...
				pstar = this.PStar(this.m_Distances[i], scale);
				tprob = pstar / this.m_ActualCount;
				avgprob += tprob;
				if (pstar < minprob) {
					minprob = pstar;
				}
				...
				for (k = 0; k <= NUM_RAND_COLS; k++) {
					...
					pseudoClassProbs[k][this.m_RandClassCols[k][i]] += tprob;
				}
			}
		}
	 ...
	 */
	private double PStar(final double x, final double scale) {
	 ...
	 */
	public void setMissingMode(final int mode) {
	 ...
	 */
	public void setBlendMethod(final int method) {
	 ...
	 */
	public void setBlendFactor(final int factor) {
