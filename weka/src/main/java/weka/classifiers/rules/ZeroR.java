...
/**
 * <!-- globalinfo-start --> Class for building and using a 0-R classifier. Predicts the mean (for a numeric class) or the mode (for a nominal class).
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class ZeroR extends AbstractClassifier implements WeightedInstancesHandler, Sourcable {
		...
		for (Instance instance : instances) {
			...
			if (!Utils.isMissingValue(classValue)) {
				if (instances.classAttribute().isNominal()) {
					this.m_Counts[(int) classValue] += instance.weight();
				} else {
	...
	public double[] distributionForInstance(final Instance instance) throws Exception {
		...
		if (this.m_Counts == null) {
			...
			result[0] = this.m_ClassValue;
			return result;
		} else {
			return this.m_Counts.clone();
		}
	}
