...
/**
 * <!-- globalinfo-start --> Class for building and using a 1R classifier; in other words, uses the
 * minimum-error attribute for prediction, discretizing numeric attributes. For more information,
 * see:<br/>
 * <br/>
...
public class OneR extends AbstractClassifier implements TechnicalInformationHandler, Sourcable {
  ...
  public String globalInfo() {
    ...
    return "Class for building and using a 1R classifier; in other words, uses " + "the minimum-error attribute for prediction, discretizing numeric "
  ...
  private class OneRRule implements Serializable, RevisionHandler {
     ...
     */
    public OneRRule(final Instances data, final Attribute attribute) throws Exception {
     ...
     *           if something goes wrong
     */
    public OneRRule(final Instances data, final Attribute attribute, final int nBreaks) throws Exception {
      ...
      this.m_breakpoints = new double[nBreaks - 1]; // last breakpoint is infinity
    }
  ...
  public double classifyInstance(final Instance inst) throws Exception {
    ...
    if (this.m_rule.m_attr.isNominal()) {
      ...
      while (v < this.m_rule.m_breakpoints.length && inst.value(this.m_rule.m_attr) >= this.m_rule.m_breakpoints[v]) {
        v++;
      }
    }
  ...
  public void buildClassifier(final Instances instances) throws Exception {
    ...
    while (enu.hasMoreElements()) {
      ...
      try {
        OneRRule r = this.newRule(enu.nextElement(), data);
   ...
   */
  public OneRRule newRule(final Attribute attr, final Instances data) throws Exception {
    ...
    if (attr.isNominal()) {
      r = this.newNominalRule(attr, data, missingValueCounts);
    } else {
      r = this.newNumericRule(attr, data, missingValueCounts);
    }
   ...
   *           if the rule can't be built successfully
   */
  public OneRRule newNominalRule(final Attribute attr, final Instances data, final int[] missingValueCounts) throws Exception {
   ...
   *           if the rule can't be built successfully
   */
  public OneRRule newNumericRule(final Attribute attr, Instances data, final int[] missingValueCounts) throws Exception {
    ...
    while (it.hasNext()) {
      ...
      // classes the same?
          ...
          // bucket not large enough?
              (oldDist[Utils.maxIndex(oldDist)] < this.m_minBucketSize))) {
  ...
  public void setOptions(final String[] options) throws Exception {
    ...
    if (bucketSizeString.length() != 0) {
      this.m_minBucketSize = Integer.parseInt(bucketSizeString);
    } else {
  ...
  public String[] getOptions() {
    ...
    Vector<String> options = new Vector<>(1);

    options.add("-B");
    options.add("" + this.m_minBucketSize);
  ...
  /**
   * Returns a string that describes the classifier as source. The classifier will be contained in a
   * class with the given name (there may be auxiliary classes), and will contain a method with the
   * signature:
   *
   * <pre>
   * <code>
   ...
   */
  public void setMinBucketSize(final int v) {
