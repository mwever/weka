...
public class Discretize extends Filter implements SupervisedFilter, OptionHandler, WeightedInstancesHandler, WeightedAttributesHandler, TechnicalInformationHandler {
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector
        .addElement(new Option("\tWhen generating binary attributes, spread weight of old " + "attribute across new attributes. Do not give each new attribute the old weight.\n\t",
  ...
  public String globalInfo() {
    ...
    return "An instance filter that discretizes a range of numeric" + " attributes in the dataset into nominal attributes."
  ...
  public String invertSelectionTipText() {
    ...
    return "Set attribute selection mode. If false, only selected" + " (numeric) attributes in the range will be discretized; if"
  ...
  private static String binRangeString(final double[] cutPoints, final int j, final int precision) {
    ...
    return j == 0 ? "" + "(" + "-inf" + "-" + Utils.doubleToString(cutPoints[0], precision) + "]"
        : j == n ? "" + "(" + Utils.doubleToString(cutPoints[n - 1], precision) + "-" + "inf" + ")"
            : "" + "(" + Utils.doubleToString(cutPoints[j - 1], precision) + "-" + Utils.doubleToString(cutPoints[j], precision) + "]";
  }
  ...
  private double[] cutPointsForSubset(final Instances instances, final int attIndex, final int first, final int lastPlusOne) throws InterruptedException {
    ...
    // Check if split is to be accepted
    if ((this.m_UseKononenko && this.KononenkosMDL(priorCounts, bestCounts, numInstances, numCutPoints))
        || (!this.m_UseKononenko && this.FayyadAndIranisMDL(priorCounts, bestCounts, numInstances, numCutPoints))) {
