...
/**
 * <!-- globalinfo-start --> Two-Level Distribution approach, changes the
 * starting value of the searching algorithm, supplement the cut-off
 * modification and check missing values.<br/>
 * <br/>
...
public class TLD extends RandomizableClassifier implements OptionHandler,
  ...
  private void findCutOff(double[] pos, double[] neg) {
    ...
    /*
     * System.err.println("\n\n???Positive: "); for(int t=0; t<pOrder.length;
     * t++)
     * System.err.print(t+":"+Utils.doubleToString(pos[pOrder[t]],0,2)+" ");
     * System.err.println("\n\n???Negative: "); for(int t=0; t<nOrder.length;
     * t++)
     * System.err.print(t+":"+Utils.doubleToString(neg[nOrder[t]],0,2)+" ");
     */
    int pNum = pos.length, nNum = neg.length, /* count, NOT USED */p = 0, n = 0;
...
class TLD_Optm extends Optimization {
  ...
  /**
   * Implement this procedure to evaluate objective function to be minimized
   */
  @Override
  protected double objectiveFunction(double[] x) {
    ...
    for (int j = 0; j < numExs; j++) {
      ...
      if (Double.isNaN(xBar[j])) {
        continue; // All missing values
      }

      NLL += 0.5
        * (b + num[j])
        * Math.log((1.0 + num[j] * w) * (a + sSq[j]) + num[j] * (xBar[j] - m)
          * (xBar[j] - m));
  ...
  protected double[] evaluateGradient(double[] x) {
    ...
    for (int j = 0; j < numExs; j++) {
      ...
      double denorm = (1.0 + num[j] * w) * (a + sSq[j]) + num[j]
        * (xBar[j] - m) * (xBar[j] - m);

      da += 0.5 * (b + num[j]) * (1.0 + num[j] * w) / denorm - 0.5 * b / a;

      db += 0.5 * Math.log(denorm) - 0.5 * Math.log(1.0 + num[j] * w) - 0.5
        * Math.log(a);
      ...
      if (num[j] / 2.0 > halfNum) {
        db -= 0.5 * diffFstDervLnGamma(0.5 * b);
      }

      dw += 0.5 * (b + num[j]) * (a + sSq[j]) * num[j] / denorm - 0.5
        * (b + num[j] - 1.0) * num[j] / (1.0 + num[j] * w);
  ...
  /**
   * Subclass should implement this procedure to evaluate second-order gradient
  ...
  protected double[] evaluateHessian(double[] x, int index) {
    ...
    case 0: // a
      ...
      for (int j = 0; j < numExs; j++) {
        ...
        h[0] += 0.5 * b / (a * a) - 0.5 * (b + num[j]) * (1.0 + num[j] * w)
          * (1.0 + num[j] * w) / (denorm * denorm);

        h[1] += 0.5 * (1.0 + num[j] * w) / denorm - 0.5 / a;

        h[2] += 0.5 * num[j] * num[j] * (b + num[j]) * (xBar[j] - m)
          * (xBar[j] - m) / (denorm * denorm);

        h[3] -= num[j] * (b + num[j]) * (m - xBar[j]) * (1.0 + num[j] * w)
          / (denorm * denorm);
      }
      break;
    ...
    case 1: // b
      ...
      for (int j = 0; j < numExs; j++) {
        ...
        int halfNum = ((int) num[j]) / 2;
        for (int z = 1; z <= halfNum; z++) {
          h[1] += 1.0 / ((b + num[j] - 2.0 * z) * (b + num[j] - 2.0 * z));
        }
        if (num[j] / 2.0 > halfNum) {
          h[1] -= 0.25 * diffSndDervLnGamma(0.5 * b);
        }

        h[2] += 0.5 * (a + sSq[j]) * num[j] / denorm - 0.5 * num[j]
          / (1.0 + num[j] * w);
    ...
    case 2: // w
      ...
      for (int j = 0; j < numExs; j++) {
        ...
        h[0] += 0.5 * num[j] * num[j] * (b + num[j]) * (xBar[j] - m)
          * (xBar[j] - m) / (denorm * denorm);

        h[1] += 0.5 * (a + sSq[j]) * num[j] / denorm - 0.5 * num[j]
          / (1.0 + num[j] * w);

        h[2] += 0.5 * (b + num[j] - 1.0) * num[j] * num[j]
          / ((1.0 + num[j] * w) * (1.0 + num[j] * w)) - 0.5 * (b + num[j])
          * (a + sSq[j]) * (a + sSq[j]) * num[j] * num[j] / (denorm * denorm);

        h[3] -= num[j] * num[j] * (b + num[j]) * (m - xBar[j]) * (a + sSq[j])
          / (denorm * denorm);
      }
      break;
    ...
    case 3: // m
      ...
      for (int j = 0; j < numExs; j++) {
        ...
        h[0] -= num[j] * (b + num[j]) * (m - xBar[j]) * (1.0 + num[j] * w)
          / (denorm * denorm);

        h[1] += num[j] * (m - xBar[j]) / denorm;

        h[2] -= num[j] * num[j] * (b + num[j]) * (m - xBar[j]) * (a + sSq[j])
          / (denorm * denorm);

        h[3] += num[j]
          * (b + num[j])
          * ((1.0 + num[j] * w) * (a + sSq[j]) - num[j] * (m - xBar[j])
            * (m - xBar[j])) / (denorm * denorm);
      }
    }
