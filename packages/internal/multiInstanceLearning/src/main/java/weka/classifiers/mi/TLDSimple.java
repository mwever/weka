...
/**
 * <!-- globalinfo-start --> A simpler version of TLD, mu random but sigma^2
 * fixed and estimated via data.<br/>
 * <br/>
...
public class TLDSimple extends RandomizableClassifier implements OptionHandler,
  ...
  public void buildClassifier(Instances exs) throws Exception {
    ...
    /*
     * for(int x=0, y=0; x<m_Dimension; x++, y++){ if((x==exs.classIndex()) ||
     * (x==exs.idIndex())) y++;
     * 
  ...
  public double[] distributionForInstance(Instance ex) throws Exception {
    ...
    distribution[0] = 1 / (1 + Math.exp(logOdds)); // Prob. for class 0
                                                   // (negative)
  ...
  private double likelihoodRatio(double[] n, double[] xBar) {
    ...
    for (int x = 0; x < m_Dimension; x++) {
      ...
      double llp = Math.log(w * n[x] + m_SgmSqP[x]) + n[x] * (m - xBar[x])
        * (m - xBar[x]) / (w * n[x] + m_SgmSqP[x]);
      LLP -= llp;

      // Log-likelihood for negative
      w = m_ParamsN[2 * x];
      m = m_ParamsN[2 * x + 1];
      double lln = Math.log(w * n[x] + m_SgmSqN[x]) + n[x] * (m - xBar[x])
        * (m - xBar[x]) / (w * n[x] + m_SgmSqN[x]);
      LLN -= lln;
  ...
  private void findCutOff(double[] pos, double[] neg) {
    ...
    while ((p < pNum) && (n < nNum)) {
       ...
       * //if(pos[pOrder[p]]<neg[nOrder[n]]) // next = pos[pOrder[p]];
       * //m_Cutoff = (split+next)/2.0; m_Cutoff = split; }
       */
...
class TLDSimple_Optm extends Optimization {
  ...
  protected double[] evaluateGradient(double[] x) {
    ...
    for (int j = 0; j < numExs; j++) {
      ...
      dw += num[j] / (w * num[j] + sSq) - num[j] * num[j] * (m - xBar[j])
        * (m - xBar[j]) / ((w * num[j] + sSq) * (w * num[j] + sSq));
  ...
  protected double[] evaluateHessian(double[] x, int index) {
    ...
    case 0: // w
      ...
      for (int j = 0; j < numExs; j++) {
        ...
        h[0] += 2.0 * Math.pow(num[j], 3) * (m - xBar[j]) * (m - xBar[j])
          / Math.pow(w * num[j] + sSq, 3) - num[j] * num[j]
          / ((w * num[j] + sSq) * (w * num[j] + sSq));

        h[1] -= 2.0 * (m - xBar[j]) * num[j] * num[j]
          / ((num[j] * w + sSq) * (num[j] * w + sSq));
      }
      break;
