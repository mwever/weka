...
public class RandomizedDataSparkJob extends SparkJob implements
  ...
  /**
   * If true then assume the last attribute is the class (when not explicitly
   * set)
   */
  ...
  public Enumeration<Option> listOptions() {
    ...
    result.add(new Option(
      "\tNumber of instances to be in each split. Use either this "
        + "option or -num-splits, but not both.", "num-instances-per-split", 1,
      "-num-instances-per-split <integer>"));
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    String numRandomlyShuffledSplits = Utils.getOption("num-splits", options);
    if (numRandomlyShuffledSplits.length() > 0) {
      setNumRandomlyShuffledSplits(numRandomlyShuffledSplits);
      setNumInstancesPerShuffledSplit("");
    } else {
      String numInstancesPerSplit =
        Utils.getOption("num-instances-per-split", options);
      if (numInstancesPerSplit.length() > 0) {
  ...
  public String[] getJobOptionsOnly() {
    ...
    if (!DistributedJobConfig.isEmpty(getNumInstancesPerShuffledSplit())) {
      options.add("-num-instances-per-split");
  ...
  public boolean runJobWithContext(JavaSparkContext sparkContext)
    ...
    if (getDataset(TRAINING_DATA) != null
      ...
      inputData = (JavaRDD<Instance>) getDataset(TRAINING_DATA).getRDD();
    ...
    if (inputData == null) {
      ...
      inputData = (JavaRDD<Instance>) d.getRDD();
    ...
    if (summaryAttOrig == null) {
      throw new DistributedWekaException(
        "Was unable to find a summary attribute for "
    ...
    if (!DistributedJobConfig.isEmpty(getNumRandomlyShuffledSplits())) {
      ...
      if (numInsts > totalNumInstances) {
        throw new DistributedWekaException(
          "Can't have more instances per split "
